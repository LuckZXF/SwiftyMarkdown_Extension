    ...
    public func process( _ string : String ) -> [SwiftyLine] {
        ...
        for  heading in lines {
            ...
            if let existentPrevious = input.lineStyle.styleIfFoundStyleAffectsPreviousLine(), foundAttributes.count > 0 {
                ...
                continue
            } else if let existent = input.lineStyle.styleIfFoundStyleAffectsPreAndBackLine() ,foundAttributes.count > 0 {
                ...
                if let lineStyle: MarkdownLineStyle = updatedPrevious.lineStyle as? MarkdownLineStyle, lineStyle == .table {
                    ...
                    } else if updatedPreviousLineStyle != .table {  //这种情况是表格的第一行
                        
                    } else if updatedPrevious.line == "", currentArr.count > 0 {  //这种情况是表格的内容项
                        var headerData: [String] = tableData.first!
